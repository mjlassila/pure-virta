...
declare function local:check-publisher($record) {
  if($record[JulkaisutyyppiKoodi contains text {'A3', 'B2', 'C1', 'C2', 'D2', 'D4', 'D5', 'D6', 'E2', 'E3'} any and empty($record/KustantajanNimi)]) then
...
declare function local:check-isbn($record) {
  if($record[JulkaisutyyppiKoodi contains text {'A3','B2','C1','D5','D5','E2'} any and (count($record/ISBN) = 0 or $record/ISBN contains text 'ERROR' using fuzzy)]) then
  <isbn_missing>true</isbn_missing>
};

declare function local:check-isbn-issn($record) {
  if($record[JulkaisutyyppiKoodi contains text {'A4',"B3","B2","C2"} any and count($record/ISBN) = 0 and count($record/ISSN) = 0]) then
...
let $csv:=<csv>{
  for $record in $intermediate_csv/record
    let $errors:= 
      for $value in $record/(publisher_missing | conference_missing | d1_mag_missing | parent_pub_missing | internal_authors_missing | authors_missing | title_missing | year_missing | issn_missing | isbn_missing | isbn_or_issn_missing)
        where $value eq 'true'
        return 1
